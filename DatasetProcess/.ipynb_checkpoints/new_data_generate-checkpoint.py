...
if model=="A":
    ...
    file_train = open(ex_train, 'w')  # 将属于测试集的数据加入
    for videoid in tqdm(new_kmeans_dict.keys()):
        if videoid in train_list_id:
            flg = True
        else:
        ...
        for caption in v2c[videoid]:
            ...
            file_train.flush()
            count_train_anno += 1
    print(count_train_anno, count_val_anno)
elif model=="C":
    print("model C")
    file_train = open(ex_train, 'w')
    for videoid in tqdm(new_kmeans_dict.keys()):
        kmeans_id = new_kmeans_dict[videoid]
        if msrvtt_video2caption[videoid][0][1]:
...
#         count_train_anno += 1
    ...
    for videoid in tqdm(new_kmeans_dict.keys()):
        kmeans_id = new_kmeans_dict[videoid]
        if videoid in msrvtt_video2caption.keys():
            if msrvtt_video2caption[videoid][0][1]:
                for caption in msrvtt_video2caption[videoid]:
                    file_train.write('\t'.join([caption[0], videoid, kmeans_id]) + '\n')
                    file_train.flush()
...
#                     count_train_anno += 1
            ...
            for caption in v2c[videoid]:
                ...
                file_train.flush()
                count_train_anno += 1
        elif videoid in didemo_video2caption.keys():
            for caption in didemo_video2caption[videoid]:
                file_train.write('\t'.join([caption, videoid, kmeans_id]) + '\n')
                file_train.flush()
                count_train_anno += 1
        elif videoid in msvd_video2caption.keys():
            for cap in msvd_video2caption[videoid]:
                caption = " ".join(cap)
                file_train.write('\t'.join([caption, videoid, kmeans_id]) + '\n')
                file_train.flush()
                count_train_anno += 1
