    ...
    def forward(
        ...
        if labels is not None:
            if self.Rdrop > 0 and self.training:
                ...
                x = torch.arange(1, seq_len + 1, dtype=torch.float).unsqueeze(0)
                y = torch.exp(-0.5 * x)
                loss_weight = y.repeat(batch_size, 1)
                loss_weight[labels == -100] = 0
                loss_weight = loss_weight.to("cuda:0")
                ...
                if loss_weight is not None:
                    ...
                    orig_loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))  # [128, 10, 302]  @TODO:
                ...
                loss = orig_loss + self.Rdrop*dist_loss  # @TODO:
